...
$node_num = $cluster['HOSTS_UP'];
$multiply = 1000;
$cpu_num = !$showhosts ? $metrics["cpu_num"]['VAL'] / $multiply: 
		cluster_sum("cpu_num", $metrics) / $multiply;
$load_one_sum = !$showhosts ? $metrics["load_one"]['VAL'] / $multiply: 
		cluster_sum("load_one", $metrics) / $node_num / $multiply;
$load_five_sum = !$showhosts ? $metrics["load_five"]['VAL'] / $multiply: 
		cluster_sum("load_five", $metrics) / $node_num / $multiply;
$load_fifteen_sum = !$showhosts ? $metrics["load_fifteen"]['VAL'] / $multiply: 
		cluster_sum("load_fifteen", $metrics) / $node_num / $multiply;
...
# Summary graphs
#
$graph_args = "c=$cluster_url&amp;$get_metric_string&amp;st=$cluster[LOCALTIME]&amp;cs=$cs&amp;ce=$ce";
...
if ($showhosts) {
    foreach ($hosts_up as $host => $val) {
        $cpus = $metrics[$host]["cpu_num"]['VAL'] / $multiply;
        if (!$cpus) $cpus = 1;
        $load_one  = $metrics[$host]["load_one"]['VAL'] / $multiply;
        $load = ($load_one)/$cpus;
        $host_load[$host] = $load;
...
foreach ($sorted_hosts as $host => $value) {
    ...
    $host_link="\"?c=$cluster_url&amp;h=$host_url&amp;$get_metric_string&amp;cs=$cs&amp;ce=$ce\"";
    $textval = "";
    ...
    if (isset($hosts_down[$host]) and $hosts_down[$host]) {
        ...
        $graphargs .= "z=$size&amp;c=$cluster_url&amp;h=$host_url"
           ."&amp;l=$load_color"
           ."&amp;r=$range&amp;su=1&amp;st=$cluster[LOCALTIME]&amp;cs=$cs&amp;ce=$ce";
    }
